...
function globalV2Manifest({ headSha = git(ROOT, 'rev-parse', 'HEAD'), version = '0.3.8', ...overrides } = {}) {
  return {
...
function pullRequestMergeFixture() {
  ...
  git(root, 'merge', '--no-ff', '-m', 'merge candidate', 'release/0.3.8');
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      pull_request: {
        head: { ref: 'release/0.3.8', sha: headSha },
...
test('repository release metadata is aligned at the release 0.3.8 baseline', () => {
  const result = validateReleaseMetadata(ROOT);
  assert.deepEqual(result.failures, []);
  assert.equal(result.packageVersion, '0.3.8');
});
...
test('feature checks compare release versions with their base', () => {
  ...
  try {
    ...
    writeFixtureMetadata(fixture, '0.3.8');
    git(fixture, 'add', '.');
...
test('v2 release and deploy read the actual symbolic branch and reject conflicting CI refs', () => {
  ...
  try {
    ...
    assert.equal(conflicting.branch, 'release/0.3.8');
  } finally {
    fs.rmSync(manifestDir, { recursive: true, force: true });
    fs.rmSync(fixtureRoot, { recursive: true, force: true });
  }
